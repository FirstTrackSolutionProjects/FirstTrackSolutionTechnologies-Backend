...
const hrms_db = mysql.createPool({
    host: process.env.HRMS_DB_HOST,        // e.g., '127.0.0.1' or your database IP
    user: process.env.HRMS_DB_USER,        // e.g., 'root' or your MySQL username
    password: process.env.HRMS_DB_PASSWORD,    // Your MySQL password
    database: process.env.HRMS_DB_NAME,    // The database name you want to connect to
    port: process.env.HRMS_DB_PORT || 3306, // Default MySQL port is 3306
...
const connectDB = async () => {
    try {
        const connection = await hrms_db.getConnection();
...
module.exports = {
    query: (text, params) => hrms_db.query(text, params),
    connectDB,
